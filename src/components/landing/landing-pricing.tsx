  ...
  return (
    <div className="space-y-12 max-w-6xl mx-auto">
      ...
      <div className="flex items-center justify-center gap-3 text-xs sm:text-sm font-medium text-[#1B1C1C] dark:text-[#F2F0F0]">
        ...
        >
          <span
            className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow-md ring-0 transition duration-200 ease-in-out ${isYearly ? (isAr ? '-translate-x-5' : 'translate-x-5') : 'translate-x-0'
              }`}
          />
        </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-stretch">
        {plans.map((p, idx) => {
          ...
          const trialDaysCount = p.trial_days !== undefined && p.trial_days !== null
            ? Number(p.trial_days)
            : (p.slug === 'enterprise' ? 0 : 14)
          ...
          const btnText = userLoggedIn
            ...
            : trialDaysCount > 0
              ? (isAr ? `ابدأ التجربة المجانية (${trialDaysCount} يوم)` : `Start ${trialDaysCount}-Day Free Trial`)
              : isPopular
          ...
          return (
            <div
              key={p.id}
              className={`rounded-lg bg-white dark:bg-[#242424] p-8 sm:p-9 flex flex-col justify-between relative transition-all duration-200 ${isPopular
