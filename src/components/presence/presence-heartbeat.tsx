    ...
    const beat = async () => {
      ...
      try {
        ...
        if (error && !cancelled) {
          ...
          if (isNetworkError) {
            console.warn("[PresenceHeartbeat] transient network disconnect during heartbeat");
          } else {
        ...
        if (!cancelled) {
          console.warn("[PresenceHeartbeat] network error during heartbeat:", err?.message || err);
        }
      }
    };
