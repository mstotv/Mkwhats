      ...
      <div ref={scrollRef} className="flex-1 space-y-4 overflow-y-auto p-4">
        ...
        {turns.map((tItem, i) => (
          <div
            ...
            className={cn(
              'flex gap-2',
              tItem.role === 'user' ? 'justify-end' : 'justify-start',
            )}
          >
            {tItem.role === 'assistant' && (
            ...
            >
              {tItem.content && <p className="whitespace-pre-wrap">{tItem.content}</p>}
              {tItem.role === 'assistant' && tItem.handoff && (
                <p
                  className={cn(
                    'flex items-center gap-1 text-xs text-amber-500',
                    tItem.content && 'mt-1.5 border-t border-border/50 pt-1.5',
                  )}
                >
