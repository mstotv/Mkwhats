...
export async function transcribeAudioMessage(
  ...
  const { buffer, mimeType, provider, apiKey, timeoutMs = 25000 } = args
  ...
  try {
    if (provider === 'openai') {
      ...
      return await transcribeWithGemini(buffer, mimeType, apiKey, timeoutMs)
    } else {
...
async function transcribeWithGemini(
  ...
  if (!normalizedMime || normalizedMime === 'audio/ogg; codecs=opus') {
  ...
  try {
    ...
    if (!res.ok) {
      const status = res.status
      console.error(`[voice-stt][gemini] Gemini API returned status ${status}`)
      return null
    }
