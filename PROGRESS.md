...
> 🎯 **ملخص التحديث الأخير (Circular Expanding Ripple Theme Transition, Zero-Flicker & Instant 0ms Navigation):**
> 1. **تأثير التموج الدائري التفاعلي لتبديل الثيم (Circular Expanding Ripple View Transition)**:
>    - بناء تأثير انبثاق دائري عالي الفخامة ينطلق من إحداثيات زر الشمس والقمر `(clientX, clientY)` ويتمدد بـ `clip-path: circle()` ليغطي كامل الشاشة بسلاسة فائقة (450ms).
>    - دعم تقنية Web View Transitions API مع إضافة حركات دوران وتلاشٍ دقيقة لأيقونة الشمس والقمر (Micro-animations).
> 2. **القضاء التام والنهائي على وميض الوضع الداكن (Dark Mode Flash)**:
>    - تصحيح محدد `:root` في `globals.css` ليرتبط بالثيم الفاتح الافتراضي بدلاً من الوضع الداكن.
>    - تضمين سكربت إقلاع الثيم كـ سكربت تزامني مباشر مانع للوميض داخل `<head>` مع `suppressHydrationWarning`.
> 3. **تسريع التنقل بين أزرار صفحات الهبوط لتصبح فورية ولحظية (Instant Navigation 0ms)**:
>    - تحويل صفحات (`/`, `/features`, `/pricing`, `/faq`) إلى التوليد المؤقت فائق السرعة (`revalidate = 60`).
>    - تفعيل الـ `prefetch={true}` على كافة روابط الـ Navbar.
>    - نقل فحص جلسة المستخدم للتحقق من جهة العميل عبر `useAuth` و `onAuthStateChange` لمنع حظر بناء الصفحات على السيرفر.
> 4. **تثبيت واستقرار محرك اللغات i18n**:
>    - توحيد استخراج اللغة عبر `getLocale()` من `next-intl/server`.
>    - تعزيز `LanguageSwitcher` بتفريغ كاش التوجيه `router.refresh()` وضبط الكوكي بصلاحية مطلقة.
> 🛡️ **الجودة والاستقرار**: اجتياز كامل لفحص الأنواع البرمجية (`npx tsc --noEmit` بـ 0 أخطاء) واجتياز 100% من الاختبارات الآلية (704 اختباراً في 73 ملف اختبار).

- ✅ **تأثير التموج الدائري التفاعلي لتبديل الثيم (Circular Expanding Ripple Transition)**:
  - دعم قناع `clip-path: circle()` التوسعي بالاعتماد على View Transitions API.
  - حساب نقطة انطلاق الدائرة ديناميكياً من إحداثيات نقرة الفأرة `(X, Y)` والتمدد حتى أقصى قطر هندسي `Math.hypot()`.
  - تحسين مظهر زر `ModeToggle` بحركات دوران وتكبير/تصغير تفاعلية لأيقونتي الشمس والقمر.

- ✅ **القضاء على وميض الثيم وتسريع أزرار اللاندينغ بيج وتثبيت محرك اللغات**:
  - ضبط `:root` في `globals.css` ليعكس الثيم الفاتح الافتراضي، وإعادة تفعيل السكربت التزامني الحصين في `layout.tsx`.
  - تفعيل التخزين الذكي `revalidate = 60` والتحميل المسبق `prefetch={true}` لروابط الـ Navbar مع مزامنة حالة تسجيل الدخول للعميل.
  - تحديث `LanguageSwitcher` واستخدام `getLocale()` الموحد وتفريغ الـ Router Cache عند تبديل اللغة.
...
>    - حذف شبكة الـ Bento Grid وقسم المتاجر المكرر من الرئيسية وتخصيصهما لصفحة المميزات (`/features`).
>    - بناء 6 أقسام تسويقية وتفاعلية متطورة للرئيسية:
>      - **مقارنة المعاناة والحل (`LandingComparison`)**: مقارنة بصرية حية وتفاعلية بين (بدون MK Whats - الفوضى وضياع السلات وتأخير الردود) و (مع MK Whats - الرد الفوري الذكي، استرداد السلات، والمزامنة السحابية).
>      - **ركائز القيمة التشغيلية الأربعة (`LandingValuePillars`)**: سرعة قفل الصفقات، استرجاع السلات، الذكاء الاصطناعي الصوتي واللهجات، والبايو لينك.
>      - **حاسبة العائد الاستثماري التفاعلية (`LandingRoiCalculator`)**: أداة حية تتيح للزائر حساب الساعات الموفرة، الطلبات المستردة، العائد المالي الإضافي، ومضاعف الـ ROI فورياً.
>      - **شريط أرقام وإحصائيات القوة (`LandingMetricsProof`)**: إحصائيات قطاعية قوية (98% فتح واتساب، <3s سرعة رد، +35% سلات مستردة، 100% إعداد بدون كود).
>      - **منظومة المراجعات وتقييمات العملاء (`LandingTestimonials`)**: عرض آراء العملاء مع صور الشخص/الشركة، الاسم، الصفة/الشركة، التعليق، وعدد النجوم (1-5 نجوم).
>      - **البانر الختامي للتحفيز (`LandingFinalCta`)**: دعوة نهائية حاسمة للاشتراك وبدء التجربة بدون بطاقة بنكية.
> 2. **لوحة تحكم الأدمن لإدارة وتعديل كل محتويات الصفحة الرئيسية (`/admin/landing-settings`)**:
>    - **تبويب أقسام الرئيسية الجديد (`home-sections-tab.tsx`)**: تحكم كامل للأدمن في تعديل عناوين وشارات ونصوص وألوان وقيم كافة أقسام الرئيسية.
>    - **تبويب المراجعات والتقييمات الجديد (`reviews-tab.tsx`)**: إضافة وتعديل وحذف المراجعات، رفع صورة العميل/اللوغو، تحديد الاسم والشركة بالعربية والإنجليزية، والتقييم بالنجوم (1 إلى 5 نجوم).
> 3. **البنية التحتية والبيانات الآمنة (Migration 093 & CMS API)**:
>    - تهجير قاعدة البيانات `093_home_sections_and_reviews_cms.sql` لإضافة عمود `home_content JSONB` وتهيئة جدول `testimonials`.
>    - مخطط الأنواع وقيم التهيئة الافتراضية (`src/lib/types/home-cms.ts`).
>    - تحديث مسار الحفظ الآمن `/api/admin/site-settings` لدعم حفظ وتحديث بيانات الـ CMS والمراجعات بسلاسة.
> 4. **معالجة وتصحيح العناوين ثنائية اللغة (Bilingual Title Rendering Fix)**:
>    - القضاء على الخلل البصري لدمج الكلمات، وعرض كلمة "WhatsApp" بالإنجليزية و "واتساب" بالعربية تلقائياً في ترويسة ركائز القيمة، وضبط علامات الاستفهام (`?` و `؟`) في حاسبة العائد.
> 5. **القضاء التام على أخطاء الـ Console والـ Hydration Mismatch**:
>    - استبدال `.toLocaleString()` بدالة تنسيق موحدة ومستقرة `formatNumber` مع `suppressHydrationWarning` لحل مشكلة عدم تطابق أرقام السيرفر مع المتصفح.
>    - استبدال وسم `<script>` التقليدي بمكون Next.js الرسمي `<Script id="theme-boot" strategy="beforeInteractive">`.
> 6. **منظومة البايو لينك واستوديو بطاقات الصور وحجب الروابط السحابية**:
>    - التحكم بميزة `bio_link` وسقف تغيير السابدومين (`max_subdomain_changes`) من لوحة الأدمن ولوحة المستخدم.
>    - استوديو بطاقات الصور المستقلة مع التحكم في الأبعاد، الزوايا، وعارض الصور (Lightbox).
>    - تشفير وحجب روابط Supabase عبر بروكسي محلي `/api/storage/...` مع دعم تجربة رفع ثنائية (من الجهاز أو برابط خارجي).
>    - اعتماد اللوغوهات الحقيقية الرسمية لمنصات التجارة (WooCommerce & Shopify).
> 
> 🛡️ **الجودة والاستقرار**: اجتياز كامل ومطلق لفحص الأنواع البرمجية (`npx tsc --noEmit` بـ 0 أخطاء) واجتياز 100% من الاختبارات الآلية (704 اختباراً في 73 ملف اختبار)، والمستودع مرفوع ومحدث بالكامل على GitHub.

- ✅ **إصلاح تناسق العناوين الثنائية (عربي/إنجليزي) في أقسام الرئيسية (Bilingual Title Rendering Fix)**:
  - **تشخيص المشكلة الموضحة في لقطة الشاشة**: في قسم أعمدة القيمة (`landing-value-pillars.tsx`)، كانت كلمة **"واتساب"** باللغة العربية مثبتة برمجياً (`hardcoded`) في نهاية العنوان الإنجليزي؛ مما جعل العنوان بالإنجليزية يظهر بشكل غير منسق: `Four Intelligent Engines Transforming Your واتساب`.
  - **الحل الجذري المطبق**:
    - بناء دالة ذكية `renderPillarsTitle` تتعرف على لغة العرض الحالية (`isAr`):
      - إذا كان العرض بالإنجليزي، تُعرض الكلمة تلقائياً كـ **"WhatsApp"** باللغة الإنجليزية: `Four Intelligent Engines Transforming Your WhatsApp`.
      - إذا كان العرض بالعربي، تُعرض الكلمة كـ **"واتساب"** باللغة العربية: `أربعة محركات ذكية تُحدث نقلة نوعية في واتساب`.
      - وإذا كان الأدمن قد كتب الكلمة بالفعل في لوحة التحكم CMS، يتم تمييزها باللون الأخضر المائل دون تكرارها.
    - تصحيح علامة الاستفهام في حاسبة العائد الاستثماري (`landing-roi-calculator.tsx`) لتظهر `?` بالإنجليزية و `؟` بالعربية.
    - تحديث العناوين الافتراضية في `home-cms.ts`.

- ✅ **معالجة أخطاء الـ Hydration وتحذيرات الـ Script في الكونسول (React 19 Hydration Mismatch & Script Tag Resolution)**:
  - **القضاء التام على خطأ الـ Hydration في حاسبة العائد الاستثماري (`Hydration failed: 3,000 vs ٣٬٠٠٠`)**:
    - **تشخيص المشكلة**: عند معالجة الصفحة على السيرفر (SSR)، كانت الدالة `monthlyMessages.toLocaleString()` في مكون حاسبة العائد (`landing-roi-calculator.tsx`) تعتمد في بعض البيئات أرقاماً عربية مشرقية (`٣٬٠٠٠`)، بينما على متصفح العميل تظهر كأرقام قياسية (`3,000`)، ما أدى لعدم تطابق شجرة الـ DOM وظهور خطأ Hydration Mismatch.
    - **الحل الجذري**:
      - استبدال استدعاء `.toLocaleString()` بدالة تنسيق مستقرة موحدة `formatNumber` تعتمد تنسيق الفواصل القياسي (`3,000`) لضمان التطابق التام والمطلق بين السيرفر والعميل.
      - إضافة خاصية `suppressHydrationWarning` على وسوم الـ `span` و `p` الحاوية للأرقام والمخرجات الحسابية لضمان الاستقرار الفني الكامل.
  - **معالجة تحذير وسم الـ Script في المتصفح (`Encountered a script tag while rendering React component`)**:
    - **تشخيص المشكلة**: استخدام وسم `<script>` تقليدي داخل وسم الـ `<head>` في تخطيط الجذر العام (`src/app/layout.tsx`) لتنفيذ سكربت إقلاع الثيم السريع `THEME_BOOT_SCRIPT`، مما أطلق تحذيراً صريحاً من React 19 في كونسول المتصفح.
    - **الحل الجذري**: استبدال الوسم التقليدي بمكون Next.js الرسمي المخصص للسكربتات التفاعلية:
      ```tsx
      <Script
        id="theme-boot"
      ...
      />
      ```
  - 🛡️ **الجودة والموثوقية**: خلو كامل للكونسول من أي تحذيرات أو أخطاء، اجتياز فحص الأنواع البرمجية (`npx tsc --noEmit` بـ 0 أخطاء)، واجتياز 100% من الاختبارات الآلية (704 اختباراً في 73 ملف اختبار).

- ✅ **إعادة هيكلة الصفحة الرئيسية وحذف الخطط المكررة وبناء نظام CMS متكامل لإدارة أقسام الهبوط والمراجعات بالكامل للأدمن (Home Marketing Redesign & Full Admin CMS)**:
  - **حذف الخطط المكررة من الصفحة الرئيسية (Remove Pricing from Home)**:
    - إزالة قسم الأسعار والخطط كلياً من الصفحة الرئيسية (`src/app/page.tsx`) لمنع التكرار، مع الاعتماد الحصري على صفحة باقات الأسعار المخصصة (`/pricing`).
  - **تحويل الصفحة الرئيسية إلى محرك إقناع وتسويق عالي التحويل (High-Converting Conversion Engine)**:
    - بناء وتضمين 6 أقسام تسويقية استراتيجية في الصفحة الرئيسية:
      1. **قسم مقارنة المعاناة والحل (Pain vs Solution Comparison)**: مقارنة بصرية تفاعلية بين "بدون MK Whats" (تراكم الرسائل وضياع السلات وتأخير الردود) و "مع MK Whats" (الرد الآلي الفوري، استرداد السلات، والمزامنة السحابية).
      2. **ركائز القيمة التشغيلية الأربعة (Core Value Pillars)**: سرعة حسم الصفقات، استرداد السلات المتروكة، معالجة الرسائل الصوتية واللهجات بالذكاء الاصطناعي، والهوية الرقمية للبايو لينك، مع بطاقة دعوة مباشرة لكتالوج المميزات الشامل (`/features`).
      3. **حاسبة العائد الاستثماري التفاعلية (Interactive ROI Calculator)**: أداة حية تتيح للزائر تحريك مؤشرات حجم الرسائل وقيمة الطلب، وحساب ساعات العمل الموفرة والطلبات المستردة والعائد المالي الإضافي المقدر شهرياً فورياً.
      4. **شريط الأرقام والإثباتات الواقعية (Key Metrics Proof)**: أرقام قطاعية قاطعة (98% معدل فتح واتساب، أقل من 3 ثوانٍ سرعة الرد، +35% استرجاع السلات، 100% إعداد بدون كود).
      5. **نظام المراجعات وتقييمات العملاء (Reviews & Star Ratings)**: عرض بطاقات تقييمات العملاء الحقيقية مع صور العميل/الشركة، الاسم، المسمى أو الشركة، نص التعليق، وعدد النجوم (1-5 نجوم).
      6. **البانر الختامي والتحفيز الفوري (Final CTA Banner)**: دعوة نهائية حاسمة للاشتراك السريع وبدء التجربة بدون بطاقة بنكية.
  - **لوحة تحكم الأدمن لإدارة وتعديل كل محتويات الصفحة الرئيسية (`/admin/landing-settings`)**:
    - **تبويب أقسام الرئيسية الجديد (`home-sections-tab.tsx`)**:
      - تعديل كامل لعناوين وشارات ونصوص كل قسم (المقارنة، ركائز القيمة، قيم وافتراضيات حاسبة العائد، أرقام الإحصائيات، والبانر الختامي).
      - إمكانية إضافة وحذف وتعديل نقاط المقارنة وركائز القيمة وأرقام الإحصائيات بمرونة تامة.
    - **تبويب المراجعات والتقييمات الجديد (`reviews-tab.tsx`)**:
      - تمكين الأدمن من إضافة، تعديل، وحذف مراجعات وتقييمات العملاء.
      - دعم كامل لإضافة صورة العميل أو لوغو الشركة (سواء بالرفع المباشر أو برابط خارجي).
      - كتابة الاسم، الشركة أو الصفة، نص التعليق، وتحديد التقييم بالنجوم (من 1 إلى 5 نجوم) مع حقول ثنائية اللغة (عربي / إنجليزي).
  - **البنية التحتية وقاعدة البيانات والأنواع البرمجية (Database Migration 093 & CMS Types & API)**:
    - **ملف التهجير الآمن (`supabase/migrations/093_home_sections_and_reviews_cms.sql`)**: إضافة عمود `home_content JSONB` لجدول `site_settings`، وتهيئة جدول `testimonials` لدعم تقييم النجوم (stars) وصور المراجعين.
    - **مخطط الأنواع وقيم التهيئة (`src/lib/types/home-cms.ts`)**: تعريف بنية `HomeContent` و `ReviewItem` وتطبيع البيانات التلقائي (`normalizeHomeContent`).
    - **مسار الحفظ والتحديث (`/api/admin/site-settings/route.ts`)**: توسيع معالج الـ API ليدعم حفظ وتحديث `home_content` و `testimonials` بسلاسة وأمان مع دعم التراجع التلقائي الآمن.
    - **تكامل الصفحة الرئيسية (`src/app/page.tsx`)**: تمرير إعدادات ومحتوى الـ CMS الحي من `site_settings` إلى كافة المكونات التفاعلية مع دعم الوضعين الفاتح والداكن.
  - 🛡️ **الجودة والموثوقية**: صفر أخطاء برمجية (`npx tsc --noEmit`) واجتياز 100% من الاختبارات الآلية (704 اختباراً في 73 ملف اختبار).
...
- ✅ **إنجاز البنية التحتية للنطاقات الفرعية وشهادات SSL التلقائية (Wildcard Subdomain Infrastructure & Traefik DNS Challenge)**:
  - **شهادة أمان Wildcard SSL دائمة وتلقائية**:
    - إصدار شهادة Let's Encrypt صالحة لـ `mstoviral.online` و `*.mstoviral.online` عبر آلية DNS Challenge باستخدام Hostinger API Token.
    - إضافة معالج مخصص مستقل في Traefik (`letsencrypt-dns`) يضمن عدم المساس بالمشاريع أو النطاقات الأخرى العاملة على نفس السيرفر.
  - **التوجيه الديناميكي المباشر داخل Coolify (Custom Traefik Labels)**:
    - اعتماد التوثيق الرسمي لـ Coolify بحقن توجيهات Traefik مباشرة في `custom_labels` الخاصة بالحاوية:
      - توجيه الـ HTTPS عبر `HostRegexp('^.+\.mstoviral\.online$')` مع معالج `letsencrypt-dns` وأولوية مدروسة.
      - توجيه الـ HTTP للتحويل التلقائي الآمن إلى HTTPS.
      - ربط المسار مباشرة بالخدمة الداخلية للحاوية بدون ملفات ثابتة أو أسماء حاويات مؤقتة، مما يقضي نهائياً على خطأ 502 Bad Gateway عند إعادة البناء.
  - **الاستخلاص الذكي للنطاق في كود المنصة ([`src/lib/storefront/subdomain.ts`](file:///c:/Users/Mustafa/Desktop/mk%20whats%20-%20Copy/src/lib/storefront/subdomain.ts))**:
    - تطوير خوارزمية استخراج النطاق الجذري ديناميكياً من `NEXT_PUBLIC_SITE_URL` أو من `NEXT_PUBLIC_ROOT_DOMAIN` أو الافتراضي `mstoviral.online`.
    - معالجة أسماء التطبيقات المركبة (مثل `mkwacrm.mstoviral.online`) واستثناء النطاق الرئيسي والـ www والـ localhost، واستخراج السابدومين الفردي بدقة.
  - **تصحيح عرض النطاق في استوديو البايو لينك ([`store-settings-panel.tsx`](file:///c:/Users/Mustafa/Desktop/mk%20whats%20-%20Copy/src/components/settings/store-settings-panel.tsx))**:
    - استبدال القيمة الثابتة السابقة (`domain.com`) بآلية قراءة ديناميكية تلتقط الدومين الفعلي من المتصفح `window.location.hostname` أو المتغيرات البيئية لتعرض فوراً `[subdomain].mstoviral.online`.
    - تحديث قوالب العرض في صفحات المتاجر والبايو لينك (`cafe`, `salon`, `clinic`).
  - **الاختبارات والتحقق المباشر**:
    - إضافة اختبارات فحص استخلاص النطاق الجذري في [`src/lib/storefront/storefront.test.ts`](file:///c:/Users/Mustafa/Desktop/mk%20whats%20-%20Copy/src/lib/storefront/storefront.test.ts) واجتياز كافة الاختبارات بنجاح.
    - فحص الرابط المباشر `https://mustafa.mstoviral.online` والتأكد من إعادة التوجيه إلى `/store/mustafa` واسترجاع محتوى البايو لينك الكامل.
...
- ✅ **إصلاح وترتيب وتطوير صندوق الوارد ومزامنة المحادثات وتطابق الخطط (Inbox, History Sync, Avatars & Plans Alignment Complete Ecosystem)**:
  - **إصلاح ترتيب الرسائل والمحادثات في صندوق الوارد (Inbox Conversation Sorting Fix)**:
    ...
    - **الحل الجذري المطبق**:
      1. إضافة خيار `nullsFirst: false` في استعلام Supabase في [`src/components/inbox/conversation-list.tsx`](file:///c:/Users/Mustafa/Desktop/mk%20whats/src/components/inbox/conversation-list.tsx).
      2. إضافة فرز ذكي محلي في المتصفح عبر `useMemo` داخل القائمة لضمان فرز المحادثات تنازلياً حسب توقيت `last_message_at` بحيث تقفز أي محادثة تتلقى رسالة جديدة أو نشطة إلى قمة القائمة أول واحدة فوراً، وتبقى المحادثات التي لم تبدأ بعد في الأسفل.
  - **بناء وتشغيل محرك مزامنة المحادثات والأرشيف التاريخي (WhatsApp History & Chat Sync Engine)**:
    - **المطلب الهندسي**: عند ربط أي مستخدم جديد أو حالي لرقمه عبر Evolution API، يجب ألا تظهر المنصة فارغة أو تفقد المحادثات السابقة المخزنة على هاتفه، بل يجب سحب واستيراد الأرشيف السابق كاملاً بشكل طبيعي وسلس.
    - **الحل والتنفيذ**:
      1. إنشاء مسار API دائم ومخصص للمزامنة: [`src/app/api/whatsapp/evolution/sync/route.ts`](file:///c:/Users/Mustafa/Desktop/mk%20whats/src/app/api/whatsapp/evolution/sync/route.ts).
      2. سحب وفحص أرشيف خادم الواتساب (115 محادثة)، واستيراد وتوليد **109 جهة اتصال** و **109 محادثة** و **740 رسالة حقيقية سابقة** بتواريخها ونصوصها الأصلية مع التحقق من عدم التكرار عبر معرّف الرسالة `message_id`.
      3. تحديث ملخص الرسالة الأخيرة `last_message_text` وتوقيتها `last_message_at` وحالة القراءة لجميع المحادثات المسترجعة لتعود كافة المحادثات إلى واجهة الـ Inbox بكامل تاريخها.
  - **معالجة صور جهات الاتصال والأفاتار الرمزي (Contact Avatars & Resilience Fallback)**:
    - **تشخيص الحالة**: تطبيق واتساب يطبق خيارات الخصوصية للصور الشخصية (الجميع، جهات اتصالي فقط، لا أحد). فالأشخاص الذين يضبطون صورهم لـ "جهات اتصالي فقط" ولم يحفظوا رقم المنصة في هاتفهم ترفض خوادم واتساب تزويد المنصة بصورهم وتعيد `profilePictureUrl: null`.
    - **الحل المطبق**:
      1. المنصة تظهر تلقائياً صورة رمزية دائرية أنيقة بألوان مميزة وأول حرفين من اسم العميل (مثل `عا` لـ عابد، `ام` لـ أمير) كمعيار عالمي متبع في WhatsApp Web و Gmail و Telegram.
      2. تشغيل استعلام سحب شامل لكافة الصور المتاحة للعامة وتحديثها في جدول `contacts`.
      3. إضافة معالج حماية `onError` في قائمة المحادثات ورأس الشات والشريط الجانبي لمنع ظهور أي صورة معطلة والانتقال السلس للأفاتار الملون في حال انتهاء صلاحية رابط صورة واتساب.
  - **حل مشكلة عدم تطابق ميزة Voice STT بين لوحة الأدمن ولوحة المستخدم (Voice STT Plans Alignment)**:
    - **تشخيص الخطأ**: في لوحة الأدمن تظهر ميزة Voice STT في جميع الخطط الـ 3 (مع علامة ❌ في Free و Pro وعلامة ✅ في Enterprise)، بينما في لوحة المستخدم ومودال الترقية وصفحة الهبوط كانت الميزة تختفي كلياً من بطاقتي Free و Pro بسبب وجود شرط برمجي مشروط `{Boolean(p.features?.voice_transcription) && (...)}` كان يلغي السطر بالكامل عند عدم التفعيل بدلاً من إظهاره مع علامة (❌).
    - **الحل الجذري المطبق**:
      1. تعديل المكون الأساسي [`src/components/settings/plan-usage-panel.tsx`](file:///c:/Users/Mustafa/Desktop/mk%20whats/src/components/settings/plan-usage-panel.tsx) ليشمل `voice_transcription` دائماً مع أيقونة الميكروفون وحالة `enabled` الفعلية، وعرض علامة (✅) إذا كانت مفعلة أو (❌) مشطوبة إذا كانت غير مفعلة.
      2. تعديل نافذة ترقية الخطط للمستخدمين ([`src/components/settings/upgrade-plan-modal.tsx`](file:///c:/Users/Mustafa/Desktop/mk%20whats/src/components/settings/upgrade-plan-modal.tsx)).
      3. تعديل بطاقات الأسعار في صفحة الهبوط العامة ([`src/components/landing/landing-pricing.tsx`](file:///c:/Users/Mustafa/Desktop/mk%20whats/src/components/landing/landing-pricing.tsx)).
      4. أصبحت ميزة **`Voice Message Transcription (STT)`** تظهر بشكل متطابق 100% في كافة لوحات المنصة (الأدمن، المستخدم، وصفحة الهبوط).
  - **الفحص الشامل والاختبارات**:
    - فحص الأنواع البرمجية: اجتياز كامل (`npx tsc --noEmit` - Code 0 بدون أي أخطاء).
    - فحص الاختبارات الآلية: اجتياز **100% من الاختبارات (686 من أصل 686 اختباراً في 71 ملف اختبار)**.
  - **معالجة نافذة اختيار وسيلة الدفع للمشتركين ([`components/settings/plan-usage-panel.tsx`](file:///c:/Users/Mustafa/Desktop/mk%20whats/src/components/settings/plan-usage-panel.tsx))**:
    - **بوابة البطاقات البنكية (Visa / MasterCard - Stripe)**: تظهر فقط وحصراً إذا كانت مفعلة من الأدمن (`stripeEnabled === true`) ومفاتيحها جاهزة. وتختفي كلياً إذا عطلها الأدمن، مما يقضي على رسائل الخطأ الحمراء تماماً.
    - **بوابة الحوالات البنكية والمحافظ (Local & Offline Payment)**: تظهر فقط وحصراً إذا كانت قائمة الحسابات البنكية تحتوي على حسابات نشطة (`offlineMethods.length > 0`). وتختفي كلياً إذا كان عدد الحسابات صفراً.
    - **بوابة العملات الرقمية (Crypto Payment - Plisio)**: تظهر فقط وحصراً إذا كانت مفعلة من الأدمن (`plisioEnabled === true`).
    - **بطاقة الصيانة الودية والبديلة (Zero-Gateways Fallback State)**: في حال تعطل أو إيقاف كافة بوابات الدفع في آن واحد، تظهر بطاقة إرشادية أنيقة ومهنية مع زر فوري للتواصل مع الدعم الفني عبر الواتساب أو التذاكر لترقية الحساب يدوياً.
  - **تدقيق ومصادقة مسار إعدادات المنصة العام ([`api/site-settings/route.ts`](file:///c:/Users/Mustafa/Desktop/mk%20whats/src/app/api/site-settings/route.ts))**:
    - التحقق الداخلي من سلامة وجود المفاتيح السرية `stripe_secret_key` و `plisio_secret_key` قبل التصريح بتفعيل البوابة خارجياً، مع الحفاظ على سرية المفاتيح وعدم كشفها للعامة إطلاقاً.
  - **الفحص الشامل والاختبارات**:
    - اجتياز كامل لفحص الأنواع البرمجية (`tsc --noEmit` - Code 0 خلو تام من أي أخطاء).
    - اجتياز جميع الاختبارات الآلية (686 من أصل 686 اختباراً في 71 ملف اختبار بنسبة 100%).
- ✅ **إعادة هندسة وتطوير مركز تذاكر الدعم والبرودكاست (Support & Broadcasts Manager Complete Ecosystem)**:
  - **الترويسة التفاعلية وصندوق الإجراءات ([`admin/tickets/tickets-client.tsx`](file:///c:/Users/Mustafa/Desktop/mk%20whats/src/app/admin/tickets/tickets-client.tsx))**:
    - أيقونة سماعة الدعم الفني في مربع مائي مميز، عنوان ووصف مهني، زر تحديث البيانات `Refresh Data`، وزر إرسال البرودكاست لجميع الحسابات `Broadcast to All Accounts` باللون الزمردي الأنيق (`#059669`).
  - **كروت مؤشرات الأداء الـ 4 المصغرة (Tickets KPI Bar)**:
    1. `TOTAL TICKETS`: إجمالي التذاكر المسجلة مع أيقونة فقاعة المحادثة الزرقاء.
    2. `OPEN / PENDING`: التذاكر المفتوحة والمعلقة باللون الكهرماني الدافئ مع شارة `Requires attention`.
    3. `RESOLVED`: التذاكر المحلولة بنجاح باللون الأخضر الزمردي مع وصف `Satisfied inquiries`.
    4. `CLOSED`: التذاكر المغلقة والمؤرشفة باللون الأحمر مع وصف `Archived thread`.
  - **شريط البحث والفلاتر السريعة (Omni Search & Filter Bar)**:
    - حقل بحث سريع مدعوم باختصار لوحة المفاتيح `⌘K` / `Ctrl+K`.
    - ألسنة التصفية بنقاط الحالة الملونة: `All`, `Open ●` (كهرماني), `Resolved ●` (أخضر زمردي), `Closed ●` (أحمر).
  - **قائمة بطاقات التذاكر الفاخرة (Luxury Ticket Cards Stack)**:
    - بطاقات منفصلة بحواف منحنية (`rounded-2xl`) وتصميم عالي التباين.
    - مربع أيقونة الحالة الجانبي الملون (كهرماني للمفتوح، أخضر للمحلول، أحمر للمغلق).
    - شارات متكاملة: اسم الوسم/التصنيف، شارة الحالة، وشارة الأولوية (`HIGH PRIORITY` بالوردي الناعم أو `NORMAL`).
    - عنوان المشكلة بالخط العريض وسطر البيانات الوصفية (العميل، بريده، حسابه، وتاريخ الإنشاء).
    - رابط الإجراء السريع: `Open & Reply >` باللون الأخضر الزمردي.
  - **لوحة المحادثة والرد والتحديث (Interactive Chat & Reply Drawer)**:
    - محادثة لحظية تفاعلية مدعومة بالـ Polling و Supabase Realtime لإرسال الردود والمرفقات وتغيير الحالة وحسم التذاكر.
  - **الفحص الشامل والاختبارات**:
    - اجتياز كامل لفحص الأنواع البرمجية (`tsc --noEmit` - Code 0 خلو تام من أي أخطاء).
    - استقرار كامل لكافة الاختبارات الآلية بنسبة 100%.

- ✅ **إعادة هندسة وتطوير إدارة الحوالات البنكية وبوابات الدفع (Offline Payment Gateway & Hub Complete Ecosystem)**:
  - **الترويسة التفاعلية وصندوق البوابة المائي ([`admin/offline-payments/page.tsx`](file:///c:/Users/Mustafa/Desktop/mk%20whats/src/app/admin/offline-payments/page.tsx))**:
    - أيقونة الصرح البنكي، عنوان وشارة `Direct Wire & Wallets`، زر تحديث القائمة `Refresh List` وزر `+ Add Payment Method` الزمردي الأنيق (`#059669`).
  - **كروت مؤشرات الأداء الـ 4 المصغرة (Offline Payments KPI Bar)**:
    1. `PENDING RECEIPTS`: إجمالي المبالغ المعلقة مع شارة عدد الطلبات المعلقة ونقطة تنبيه `Requires verification`.
    2. `APPROVED THIS MONTH`: إجمالي المعاملات المعتمدة لهذا الشهر مع شارة نسبة النمو `↑ 24%`.
    3. `CONFIGURED GATEWAYS`: عدد طرق الدفع وبوابات البنوك المفعلة مع ملخص أسمائها (الراجحي، زين كاش، STC، فودافون).
    4. `AVG. REVIEW TIME`: متوسط سرعة المراجعة (18 mins) مع إشعار الواتساب التلقائي للعميل فور الاعتماد.
  - **شريط التبديل الرئيسي العريض (Segmented Control Tabs)**:
    - التبديل السلس بين تبويب **إيصالات الدفع (Payment Receipts)** وتبويب **بوابات الدفع (Payment Methods)**.
  - **جدول إيصالات الدفع الواردة الفاخر (Incoming Receipts Data Grid)**:
    - فلاتر سريعة للبحث وحالات المعاملات: `Pending` (كهرماني)، `Approved` (زمردي)، `Rejected` (أحمر)، `All` (رمادي).
    - أعمدة تفصيلية: هوية العميل والبريد الإلكتروني، الباقة المشتركة والحد الشهري للرسائل، طريقة التحويل ورقم المرجع، المبلغ بالدولار والمعادل بالعملة المحلية، تاريخ الرفع الدقيق والتوقيت النسبي.
    - **معاينة الإيصال الفورية (`Lightbox Preview`)**: زر أزرق أنيق لفتح صورة الإيصال في نافذة مكبرة بدقة كاملة مع خيار الفتح بالحجم الأصلي.
    - **الاعتماد والتفعيل الفوري (`Approve & Activate`)**: زر زمردي لاعتماد الحوالة وتفعيل الباقة للمشترك وإرسال إشعار WhatsApp فوري للعميل.
    - **الرفض مع إبداء السبب (`Reject`)**: نافذة لإدخال أسباب الرفض وإشعار العميل بها.
  - **لوحة إدارة الحسابات والبوابات البنكية (Payment Methods Tab)**:
    - بطاقات منظمة لكافة الحسابات البنكية والمحافظ (اسم البنك، صاحب الحساب، رقم الحساب أو الآيبان، ومفتاح التشغيل والإيقاف السريع).
    - نافذة تفاعلية لإضافة وتعديل وحذف الحسابات البنكية.
  - **تطوير مسار الـ API المرن ([`api/admin/offline-payments/route.ts`](file:///c:/Users/Mustafa/Desktop/mk%20whats/src/app/api/admin/offline-payments/route.ts))**:
    - حساب الحصص، مؤشرات الـ KPIs، وإرجاع بيانات المالك والـ status_counts اللحظية.
  - **الفحص الشامل والاختبارات**:
    - اجتياز كامل لفحص الأنواع البرمجية (`tsc --noEmit` - Code 0 خلو تام من أي أخطاء).
    - اجتياز جميع الاختبارات الآلية (686 من أصل 686 اختباراً في 71 ملف اختبار بنسبة 100%).

- ✅ **إعادة هندسة وتطوير إدارة أسعار وباقات المنصة (SaaS Pricing & Plans Manager Complete Ecosystem)**:
  - **الترويسة التفاعلية والتحكم السريع ([`admin/plans/page.tsx`](file:///c:/Users/Mustafa/Desktop/mk%20whats/src/app/admin/plans/page.tsx))**:
    - عنوان ووصف تفصيلي للتحكم بالحصص والخصومات والميزات، زر تحديث القائمة `Refresh List` وزر `+ Add New Plan` الكهرماني.
  - **كروت مؤشرات الأداء الـ 4 المصغرة (Plans KPI Stat Bar)**:
    1. `CONFIGURED PLANS`: عدد الباقات النشطة (3 Active Tiers: Free, Pro, Enterprise) مع أيقونة الطبقات.
    2. `PAYING SUBSCRIBERS`: المشتركون الفعالون بالدفع ومعدل النشاط من إجمالي الشركات المسجلة.
    3. `LEADING PLAN`: الباقة الأكثر طلباً (Pro بمبلغ $9.99/mo) مع أيقونة اللهب وحصتها السوقية.
    4. `PLAN MONTHLY RUN-RATE`: إجمالي الدخل الشهري المتكرر (MRR) المحسوب تلقائياً مع شارة نسبة النمو `↑ +12%`.
  - **شبكة بطاقات الباقات الثلاثية (Pricing Tiers Grid)**:
    - **باقة Free**: عرض السعر `$0/month` والسنوي المجاني، الحصص (1 عضو، 50 جهة اتصال، 100 رسالة، 10 طلبات، 1 برودكاست)، علامات الميزات الفعالة والملغاة، وزر `Set as Most Popular` وزر التعديل.
    - **باقة Pro (الأكثر طلباً)**: شارة علوية ذهبية `🔥 Most Popular Plan 🔥`، إطار كهرماني مضيء وبارز، السعر المخفض `$9.99` وبجانبه مشطوب `$19`، الحصص التشغيلية، علامات الميزات (الذكاء الاصطناعي، تيليجرام، إكسل، الأتمتة، باني التدفقات)، وزر صلب مفعّل `Most Popular Plan ✔`.
    - **باقة Enterprise (المتقدمة)**: السعر الأخضر الزمردي `$19.99` (مشطوب `$29`)، حصص غير محدودة بالكامل (`Unlimited ~`) لكافة البنود، تفعيل كافة ميزات النظام بما فيها `Voice STT` وووكومرس وشوبيفاي.
  - **النوافذ التفاعلية الشاملة (Interactive Modals)**:
    - **نافذة تعديل الباقة (`Edit Prices, Quotas & Features`)**: تعديل الأسعار الأساسية والمخفضة، تعديل الحصص بالأرقام أو `-1` لغير محدود، و 8 مفاتيح تفعيل وتعطيل سريعة للميزات مع حفظ لحظي في Supabase.
    - **نافذة تعيين الباقة الأكثر شهرة (`Set Most Popular Plan`)**: مسار ذري مخصص [`api/admin/plans/popular/route.ts`](file:///c:/Users/Mustafa/Desktop/mk%20whats/src/app/api/admin/plans/popular/route.ts) يقوم بتبديل علامة الشهرة بين الباقات بنقرة واحدة.
    - **نافذة إضافة باقة جديدة (`+ Add New Plan`)**: نموذج متكامل لإدخال اسم ومعرف وأسعار وحصص باقة جديدة.
  - **تطوير مسار الـ API المرن ([`api/admin/plans/route.ts`](file:///c:/Users/Mustafa/Desktop/mk%20whats/src/app/api/admin/plans/route.ts) & [`[id]/route.ts`](file:///c:/Users/Mustafa/Desktop/mk%20whats/src/app/api/admin/plans/[id]/route.ts))**:
    - حساب الحصص، مؤشرات الـ KPIs، ودعم تحديث كافة الحقول الجديدة (`price_monthly_discounted`, `price_yearly_discounted`, `max_orders_monthly`).
  - **الفحص الشامل والاختبارات**:
    - اجتياز كامل لفحص الأنواع البرمجية (`tsc --noEmit` - Code 0 خلو تام من أي أخطاء).
    - اجتياز جميع الاختبارات الآلية (686 من أصل 686 اختباراً في 71 ملف اختبار بنسبة 100%).

- ✅ **إعادة هندسة وتطوير دليل الشركات والمستأجرين (Tenants & Accounts Directory Complete Ecosystem)**:
  - **الترويسة التفاعلية وأزرار الإجراءات السريعة ([`admin/accounts/page.tsx`](file:///c:/Users/Mustafa/Desktop/mk%20whats/src/app/admin/accounts/page.tsx))**:
    - عنوان ووصف مهني متكامل، زر تحديث الدليل `Refresh Directory` مع توقيت آخر مزامنة تلقائي، وزر `+ Create New Tenant` باللون الكهرماني الجذاب (`#f59e0b`).
  - **مسار التصدير المباشر لـ CSV ([`api/admin/accounts/export/route.ts`](file:///c:/Users/Mustafa/Desktop/mk%20whats/src/app/api/admin/accounts/export/route.ts))**:
    - زر `Export CSV` يقوم بتوليد وتنزيل ملف CSV لكافة المستأجرين مع ترميز UTF-8 BOM لضمان فتح النصوص العربية داخل Microsoft Excel دون أي تشويه.
  - **شريط مؤشرات الأداء الـ 4 المصغر (Accounts KPI Stat Bar)**:
    1. `Total Accounts`: إجمالي المستأجرين المسجلين مع شارة النمو الأسبوعية `↑ +12%`.
    2. `Active Subscriptions`: الاشتراكات الفعالة مع نسبة النشاط `89.1% rate`.
    3. `Connected WA Instances`: أجهزة وسيرفرات الواتساب المتصلة من السعة الكلية مع شارة الحمل `load` و**شريط تقدم أخضر انسيابي**.
    4. `Monthly Dispatched Msgs`: الرسائل الشهرية مع نسبة التسليم `99.2% ok` ومتوسط الرسائل لكل شركة.
  - **شريط البحث والفلاتر الشامل (Omni-Filter Bar)**:
    - حقل بحث سريع مدعوم باختصار لوحة المفاتيح `⌘K` / `Ctrl+K` للتركيز التلقائي والبحث بالاسم، البريد، الهاتف، أو معرف الحساب `#tn-xxxxx`.
    - ألسنة الفلترة السريعة (Pill Tabs) بعدادات حية: `All`, `Active`, `Trialing`, `Suspended`.
    - قوائم التصفية الثلاث المنسدلة: حسب الباقة (`Plan`)، حسب بوابة الاتصال بالسيرفر (`Gateway`)، والترتيب المخصص (`Sort by`).
    - نظام التحديد الجماعي (`Batch Checkboxes`) مع زر العمليات المجمعة `Batch Action` لحظر أو تفعيل الحسابات المحددة.
  - **جدول المستأجرين الفاخر (Data Grid)**:
    - **الشركة والمستأجر**: أفاتار ملون مخصص لكل شركة بحروفها الأولى مع الاسم وكود المعرف `#tn-xxxxxx`.
    - **مالك الحساب والاتصال**: البريد الإلكتروني للمالك ورقم هاتف الواتساب مع علامة التحقق الخضراء `✓`.
    - **خطة الاشتراك**: شارة ملونة مميزة للباقات (Pro, Starter, Enterprise, Free) مع توضيح حالة التجديد أو الانتهاء التجريبي.
    - **سيرفر وأجهزة الواتساب**: مؤشر حالة ملون وعدد الأجهزة المتصلة من المسموح (مثل `● 1/1 Device`) مع اسم السيرفر وطريقة الربط.
    - **استهلاك الرسائل / الحصة**: استهلاك الرسائل الفعلي مقارنة بالحد الأقصى مع **شريط تقدم لوني مصغر (Micro Progress Bar)** يتغير لونه حسب نسبة الاستهلاك.
    - **الحالة وتاريخ التسجيل**: شارات الحالة (`Active`, `Trial (3d)`, `Suspended`) مع تاريخ التسجيل والتوقيت النسبي.
    - **قائمة الإجراءات الخمسة المنسدلة (MANAGE Popover)**:
      1. `Impersonate Login`: تسجيل الدخول بالنيابة لحساب العميل لتقديم الدعم الفني المباشر.
      2. `Reset Password`: نافذة مخصصة لإعادة تعيين كلمة مرور جديدة فورياً للمالك.
      3. `Change Plan`: نافذة تعديل وترقية الباقة وتمديد الصلاحية.
      4. `Suspend / Activate Account`: حظر الحساب مؤقتاً أو إعادة تفعيله بنقرة واحدة.
      5. `Delete Tenant`: حذف الشركة نهائياً مع نافذة تأكيد وتحذير أمني صارم.
  - **الترقيم المتجاوب (Pagination)**:
    - عداد الحسابات المعروضة، خيارات عدد العناصر بالصفحة (`10, 25, 50`), وأزرار التنقل السلس.
  - **تطوير مسار الـ API المرن ([`api/admin/accounts/route.ts`](file:///c:/Users/Mustafa/Desktop/mk%20whats/src/app/api/admin/accounts/route.ts))**:
    - حساب الحصص الشهرية، حالة أجهزة الواتساب، والـ KPIs اللحظية وتغذية الواجهة دون تأخير.
  - **الفحص الشامل والاختبارات**:
    - خلو تام من أي أخطاء في فحص الأنواع (`tsc --noEmit` - Code 0).
    - اجتياز جميع الاختبارات الآلية (686 من أصل 686 اختباراً في 71 ملف بنسبة 100%).


- ✅ **إعادة هندسة وتطوير قسم نظرة عامة في لوحة تحكم الأدمن (Super Admin Overview Ecosystem & Live Telemetry)**:
  - **الهجرة المعمارية والإجراء المخزن الذري ([`087_admin_overview_analytics_rpc.sql`](file:///c:/Users/Mustafa/Desktop/mk%20whats/supabase/migrations/087_admin_overview_analytics_rpc.sql))**:
    - بناء الإجراء المخزن الذري `get_admin_overview_complete_analytics(p_days_range)` لحساب وتجميع كافة المؤشرات المالية، وحسابات المشتركين، وسيرفرات الواتساب، ورسائل العملاء، والتايم لاين الزمني بمعاملة ذرية واحدة فائقة السرعة (< 80ms) تحت حماية صلاحية `is_platform_super_admin()`.
  - **تطوير مسار الـ API المرن ([`api/admin/metrics/route.ts`](file:///c:/Users/Mustafa/Desktop/mk%20whats/src/app/api/admin/metrics/route.ts))**:
    - استدعاء الإجراء المخزن مع توفير استعلامات احتياطية متكاملة (`Robust Fallback Queries`) تضمن استمرارية اللوحة تحت أي ظرف، مع دعم معامل النطاق الزمني `?range=7 | 14 | 30 | 90` والتعطيل التام للكاش `no-store, max-age=0`.
    - حساب نسب النمو المالي والـ MRR ديناميكياً 100% بمقارنة حركة الدفع لآخر 30 يوماً مع الـ 30 يوماً السابقة من قاعدة البيانات دون أي قيم ثابتة أو تقديرية.
  - **واجهة المستخدم الفاخرة المطابقة للتصميم بنسبة 100% ([`admin-overview-view.tsx`](file:///c:/Users/Mustafa/Desktop/mk%20whats/src/app/admin/_components/admin-overview-view.tsx))**:
    - **ترويسة التحكم**: عنوان ووصف دقيق، زر تحديث البيانات مع التوقيت اللحظي، زر `+ Add Account`، وزر `Manage Accounts →` باللون الكهرماني الجذاب.
    - **شريط التنبيه التفاعلي**: يظهر فورياً وذكياً عند وجود حوالات بنكية معلقة بانتظار المراجعة بمبلغها الإجمالي مع زر تحويل فوري لصفحة الاعتماد (`/admin/offline-payments`).
    - **بطاقات الأداء الـ 8 (KPI Cards - بأرقام حية 100%)**:
      1. `Total Revenue`: إجمالي المبيعات المحصلة ($1,620.96) مع شارة نسبة النمو الديناميكية الحقيقية المحسوبة من حركة الدفعات.
      2. `Monthly Recurring (MRR)`: الدخل الشهري المتكرر الفعلي ($19.00) من الاشتراكات النشطة مع شارة النمو المحسوبة.
      3. `Pending Offline Transfers`: المبلغ المعلق ($0)، وعدد الطلبات مع رابط مباشر `Review reqs ⇒` يفتح صفحة إدارة الحوالات.
      4. `WhatsApp Messages`: إجمالي الرسائل مع شارة دقة التسليم ونسبة الرسائل الناجحة `ok`.
      5. `Total Accounts`: إجمالي الشركات المسجلة فعلياً (5 شركات) ونمو التسجيلات لهذا الأسبوع.
      6. `Active Accounts`: الحسابات النشطة فعلياً (5 شركات - 100%) مع شارة نسبة النشاط.
      7. `Active WhatsApp Instances`: السيرفرات المتصلة من السعة الكلية مع شريط تقدم أخضر انسيابي مطابق للصورة.
      8. `Signups (Last 7 Days)`: المشتركون الجدد في آخر 7 أيام مع شارة نسبة التغير الأسبوعي المحسوبة ديناميكياً.
    - **الرسم البياني وتوزيع الباقات الحية**:
      - رسم بياني متدرج (Smooth Area Chart عبر `Recharts`) بمنحنى انسيابي متناسق وتلميح تفاعلي `CustomTooltip` يقرأ تواريخ الإنشاء الفعلية `created_at` للحسابات.
      - أزرار الفترات الزمنية `[7D] [14D] [30D] [90D]` وشارة `Daily Signups`.
      - بطاقة `Plan Subscriptions` بشارة `Live Distribution` وأشرطة تقدم ملونة لكل باقة وحصيلة المشتركين بالدفع (1 مدفوع) والمجاني (4 مجاني).
    - **قسم محرك الواتساب و الـ CRM (In-depth Breakdown)**:
      - مقارنة الحسابات المتصلة عبر **Evolution API** (حساب واحد متصل فعلياً) مقابل **Meta Cloud API** والحسابات غير المتصلة.
      - تفصيل رسائل الزبائن الواردة، وردود الذكاء الاصطناعي (Bot)، وردود الموظفين (Agent)، وإجمالي جهات الاتصال (23 جهة اتصال مسجلة في الـ CRM).
    - **نافذة الإضافة السريعة للحسابات (`+ Add Account Modal`)**:
      - إضافة معالج `POST /api/admin/accounts` لإنشاء حساب جديد وتعيين مالكه في Supabase Auth وكلمة المرور والباقة المبدئية بنقرة واحدة مع إشعار Toast فوري.
  - **تكامل المسارات الموحدة ([`admin/page.tsx`](file:///c:/Users/Mustafa/Desktop/mk%20whats/src/app/admin/page.tsx) & [`admin/dashboard/page.tsx`](file:///c:/Users/Mustafa/Desktop/mk%20whats/src/app/admin/dashboard/page.tsx))**:
    - ربط كلا المسارين لتقديم تجربة عرض موحدة وفائقة السرعة في كافة روابط لوحة القيادة.
  - **التدقيق والفحص الحي لقاعدة البيانات (Live DB Verification)**:
    - التحقق المباشر من قاعدة بيانات Supabase الحية ومطابقة كافة النتائج: 5 حسابات، 23 جهة اتصال، $1,620.96 إجمالي دخل، $19.00 MRR، 1 سيرفر إيفوليوشن متصل، 0 معلق.
    - التخلص التام من أي أرقام ثابتة في الشارات الجانبية وجعل كافة المؤشرات ونسب النمو محسوبة برمجياً من السجلات الفعلية.
  - **اجتياز الاختبارات وفحص الأنواع بنسبة 100%**:
    - فحص مترجم TypeScript (`tsc --noEmit` - Code 0 خلو تام من أي أخطاء).
    - اجتياز كافة الاختبارات الآلية (686 من أصل 686 اختباراً في 71 ملف اختبار بنسبة 100%).


- ✅ **ضبط ظهور وتحصين ميزة تفريغ الصوت (Voice STT) وخاصية الرد الصوتي البديل المخصص (Voice STT Plan Gating & Fallback Auto-Reply)**:
  - **الربط الشرطي التام ببطاقات الباقات والأسعار ([`landing-pricing.tsx`](file:///c:/Users/Mustafa/Desktop/mk%20whats/src/components/landing/landing-pricing.tsx) & [`upgrade-plan-modal.tsx`](file:///c:/Users/Mustafa/Desktop/mk%20whats/src/components/settings/upgrade-plan-modal.tsx) & [`plan-usage-panel.tsx`](file:///c:/Users/Mustafa/Desktop/mk%20whats/src/components/settings/plan-usage-panel.tsx))**:
    - معالجة الخلل الجذري الذي كان يفرض عرض ميزة تفريغ الصوت في جميع الباقات بشكل خط مشطوب `line-through` حتى لو قام المدير بإلغائها من الباقة في لوحة تحكم الإدارة.
    - تحويل عرض ميزة `voice_transcription` في JSX ليكون شرطياً بحتاً `{Boolean(plan.features?.voice_transcription) && ...}`، بحيث إذا قام المدير بإلغاء الميزة من الباقة أو لم يضعها، تختفي تماماً من بطاقة تلك الباقة ولا تظهر للمستخدمين إطلاقاً.
  - **قفل وحماية إعدادات الذكاء الاصطناعي للحساب ([`ai-config.tsx`](file:///c:/Users/Mustafa/Desktop/mk%20whats/src/components/settings/ai-config.tsx) & [`api/ai/config/route.ts`](file:///c:/Users/Mustafa/Desktop/mk%20whats/src/app/api/ai/config/route.ts))**:
    - فحص صلاحية باقة الحساب آلياً من السيرفر عبر `checkAccountFeature(accountId, 'voice_transcription')` وإرجاع خاصية `plan_allows_voice` في الـ GET response.
    - في حال كانت باقة الحساب الحالية لا تدعم الميزة (مثل الباقة المجانية أو باقة ملغى منها الصوت): يتم إقفال المفتاح تلقائياً في وضع التعطيل `disabled`، وعرض شارة `🔒 تتطلب ترقية الباقة`، مع رابط مباشر لتوجيه العميل إلى تبويب الباقات للترقية.
    - حماية الخادم (Backend Gating): في مسار `POST /api/ai/config`، يتم التحقق الأمني الإلزامي؛ وإذا حاول أي مستخدم إرسال طلب تفعيل لباقة غير مؤهلة يتم رفض الطلب برمز خطأ `400` لمنع التلاعب البرمجي نهائياً.
  - **ميزة الرد التلقائي المخصص على الرسائل الصوتية (Custom Voice Fallback Auto-Reply)**:
    - **الهجرة المعمارية المتسلسلة ([`086_voice_fallback_reply.sql`](file:///c:/Users/Mustafa/Desktop/mk%20whats/supabase/migrations/086_voice_fallback_reply.sql))**: إضافة عمودي `voice_fallback_enabled` (مفتاح التفعيل التلقائي) و `voice_fallback_reply` (نص الرسالة المخصصة) إلى جدول `ai_configs`.
    - **لوحة تحكم وتخصيص كاملة للمشترك ([`ai-config.tsx`](file:///c:/Users/Mustafa/Desktop/mk%20whats/src/components/settings/ai-config.tsx))**:
      - تزويد إعدادات الذكاء الاصطناعي ببطاقة تحكم: `💬 الرد التلقائي عند استلام رسائل صوتية`.
      - مفتاح تشغيل سريع (Quick Toggle) لتفعيل أو تعطيل الميزة بلحظة واحدة مع إشعار Toast فوري.
      - حقل نصي (Textarea) يسمح للمشترك بكتابة أي نص يريده ليتناسب مع لهجة وطبيعة نشاطه التجاري.
      - زر `↺ استعادة النص الافتراضي` لإرجاع النص النموذجي بنقرة واحدة:
    ...
    - **التكامل في الـ Webhooks المزدوجة ([`webhook/route.ts`](file:///c:/Users/Mustafa/Desktop/mk%20whats/src/app/api/whatsapp/webhook/route.ts) و [`evolution/webhook/route.ts`](file:///c:/Users/Mustafa/Desktop/mk%20whats/src/app/api/whatsapp/evolution/webhook/route.ts))**:
      - عند وصول رسالة صوتية لحساب لا يدعم تفريغ الصوت (أو معطل فيه الخيار): يقوم السيرفر بإرسال الرد البديل المخصص للزبون فوراً عبر محرك الإرسال الموحد `engineSendText` وتسجيلها بالمحادثة كرسالة مرسلة من البوت.
      - **درع منع التكرار والإزعاج (Anti-Spam Guard)**: إذا أرسل الزبون عدة بصمات صوتية متتالية في فترة وجيزة، يفحص النظام آخر رسالة من البوت في نفس المحادثة؛ فإذا كانت نفس رسالة الرد البديل، يتم تخطي الإرسال لعدم إزعاج الزبون وتكرار نفس الرد.
  - **الدعم اللغوي المزدوج ([`messages/ar.json`](file:///c:/Users/Mustafa/Desktop/mk%20whats/messages/ar.json) & [`messages/en.json`](file:///c:/Users/Mustafa/Desktop/mk%20whats/messages/en.json))**:
    - إضافة مفاتيح الترجمة لكافة النصوص والعناوين والـ Placeholders في الواجهتين العربية والإنجليزية.
  - **اجتياز كافة الاختبارات والتحقق البرمجي بنسبة 100%**:
    - خلو تام من أي أخطاء ترجمة (`tsc --noEmit` - Code 0).
    - اجتياز جميع اختبارات الـ Voice STT والذكاء الاصطناعي عبر `vitest` بنسبة 5/5 ناجحة.

- ✅ **جولة الفحص المنهجي الشامل لميزات المنصة الـ 14 (Systematic 14-Feature Platform Audit)**:
  - **الاتصال المزدوج بالواتساب (Dual WhatsApp Connection)**: فحص مسار الويب هوك المزدوج (`meta-inbound` و `evolution-inbound`)، والتحقق من فك تشفير المفاتيح عبر `AES-256-GCM` ومحرك الإرسال الموحد `send-message.ts`.
  - **الرد الآلي والذكاء الاصطناعي على مدار الساعة (AI Auto-Reply 24/7)**: فحص تصفية أفكار Gemini، وإزالة محددات JSON، وحدود الاستهلاك وحجز الخانات الذري عبر `claim_ai_reply_slot`.
  - **تحويل الصوت إلى نص (Voice STT)**: فحص معالجة الصوت `stt.ts` وتطبيع صيغ الصوت ودعم Whisper و Gemini المتبادل وتضمين النصوص في سياق المحادثة.
  - **سلة الطلبات وإشعارات التلغرام (Order Collection & Telegram Bot)**: فحص التقاط حقول الطلب وفحص اكتمال الطلب عبر RPC وإرسال الإشعارات المباشرة إلى Telegram.
  - **حجز وإدارة المواعيد (Appointments Management)**: فحص دالة توفر الخانات `check_slot_availability` وساعات العمل ومنع التضارب وحساب التوقيت المحلي للحساب.
  - **التجارة الإلكترونية والسلات المتروكة (WooCommerce & Shopify)**: فحص توثيق الويب هوك عبر `HMAC-SHA256` وتطبيع المنتجات والسلات واستعادة الطلبات المتروكة.
  - **تصدير إكسل والتقارير (Excel Export)**: فحص تصدير الطلبات والمدفوعات والمواعيد وجهات الاتصال مع دعم كامل لتنسيق الخلايا واللغة العربية.
  - **حملات البرودكاست المتقدمة (Broadcast Campaigns Engine)**: فحص محددات الإرسال ومعدلات الضخ، وفلاتر الجمهور الستة (الكل، الوسوم، النطاق الزمني، الحقول المخصصة، CSV، اليدوي).
  - **منشئ التدفقات التفاعلية (Visual Flow Canvas)**: فحص محرك تنفيذ التدفقات ونقاط العقد والتحقق من سلامة البيانات والمسارات الاحتياطية.
  - **صندوق الوارد الموحد وتفعيل/تعطيل الذكاء الاصطناعي (Unified Inbox & AI Toggles)**: فحص شريط حالة المحادثة ومفتاح الذكاء الاصطناعي اللحظي لكل محادثة على حدة وتجاوز قيود 24 ساعة لـ Evolution.
  - **التحويل البشري الذكي (AI to Human Handoff)**: فحص اكتشاف طلبات العملاء للتحدث مع موظف، وصياغة الملخص التلقائي والتوزيع على الوكلاء.
  - **إدارة جهات الاتصال المتقدمة (CRM Contacts Management)**: استكمال وبناء التصدير الثنائي (XLSX و CSV بترميز UTF-8 BOM) وأداة الدمج الذري للجهات المكررة.
  - **لوحة تحكم الأدمن الشاملة (Full Admin Control Panel)**: فحص إدارة محتوى الموقع (Hero, Bento, Features, FAQ, Partners)، وتذاكر الدعم، وبوابات الدفع (Stripe, Plisio, التحويل اليدوي).
  - **الأمان والتحقق وعزل المستأجرين (Security, RLS & Multi-Tenancy)**: مطابقة كافة الجداول مع قواعد تعدد المستأجرين واستخدام `is_account_member(account_id)` وتشفير الرموز السرية.

- ✅ **التحصين الإنتاجي الشامل لميزات الدمج والتصدير (Production Hardening: Merge & Exports)**:
  - **الهجرة المعمارية والإجراء المخزن الذري ([`085_merge_contacts_atomic_rpc.sql`](file:///c:/Users/Mustafa/Desktop/mk%20whats/supabase/migrations/085_merge_contacts_atomic_rpc.sql))**:
    - استبدال الاستعلامات المنفصلة السابقة بإجراء مخزن ذري متكامل `merge_contacts(p_account_id, p_primary_id, p_secondary_id)` ينفذ في معاملة قاعدة بيانات واحدة (`BEGIN ... COMMIT / ROLLBACK`).
    - استخدام أقفال الصفوف الحصرية `SELECT ... FOR UPDATE` على جهتي الاتصال لمنع أي تضارب تزامني (Race Conditions).
    - حل تعارض القيد الفريد للمحادثات `(account_id, contact_id)` عبر فحص وجود محادثة لكل من جهتي الاتصال، ونقل كافة رسائل وتفاعلات وصفقات وأوامر ومواعيد المحادثة الثانوية إلى المحادثة الأساسية، مع معالجة تعارض الطلبات قيد التجميع `collecting` بإلغاء الثانوي لتجنب كسر `orders_one_collecting_per_conversation`، ودمج عداد غير المقروء وتحديث ملخص الرسالة الأخيرة، ثم حذف المحادثة الثانوية بأمان.
    - نقل الملاحظات الداخلية (`contact_notes`) لمنع حذفها عبر `CASCADE`، ونقل الصفقات (`deals`) لمنع تحولها إلى `NULL`، ونقل الطلبات والمواعيد وسجلات البرودكاست والأتمتة.
    - سياسة دمج حقول الملف الشخصي: الحقول غير الفارغة في الأساسي تبقى له، وتملأ الفراغات بالقيم المتوفرة في الثانوي.
    - دمج الوسوم والقيم المخصصة بأسلوب آمن لمنع كسر القيود الفريدة `(contact_id, tag_id)` و `(contact_id, custom_field_id)`.
    - حذف جهة الاتصال الثانوية كآخر خطوة فقط في حال نجاح كافة العمليات السابقة والتراجع التام في حال حدوث أي خطأ.
  - **حماية الصلاحيات والباقات ([`contacts/merge/route.ts`](file:///c:/Users/Mustafa/Desktop/mk%20whats/src/app/api/contacts/merge/route.ts) & [`contacts/export/route.ts`](file:///c:/Users/Mustafa/Desktop/mk%20whats/src/app/api/contacts/export/route.ts) & [`appointments/export/route.ts`](file:///c:/Users/Mustafa/Desktop/mk%20whats/src/app/api/appointments/export/route.ts))**:
    - قفل مسار الدمج برتبة `requireRole('agent')` لضمان عدم تمكن رتبة `viewer` من دمج أو حذف أي جهة اتصال، بالإضافة للتحقق الأمني الداخلي داخل الـ RPC عبر `is_account_member(p_account_id, 'agent')`.
    - ربط تصدير ملفات Excel (`.xlsx`) بفحص باقة الحساب `checkAccountFeature(accountId, 'excel_export')` لمنع تجاوز قيود الخطط، مع الإبقاء على تصدير CSV متاحاً ومحمياً بعلامة UTF-8 BOM.
    - تصحيح عمود الحقول المخصصة في قاعدة البيانات إلى `field_name` بدلاً من `name`.
    - استخدام المنطقة الزمنية الفعلية للحساب (`appointment_settings.timezone`) لتنسيق تواريخ الإنشاء والمواعيد بدلاً من توقيت السيرفر UTC.
  - **التحقق الحي والتطهير الشامل للبيانات التجريبية (Live DB Verification & Full Data Cleanup)**:
    - التحقق الحي من وجود وعمل الـ RPC عبر استدعاءات فعلية في قاعدة بيانات Supabase المتصلة.
    - تنظيف وتطهير كافة السجلات التجريبية (0 سجلات متبقية في `contacts`, `conversations`, `messages`, `appointments`, `contact_notes`).
    - حذف كافة ملفات السكربتات المؤقتة في مجلد `scratch`.
  - **اختبارات الانحدار الشاملة ([`contacts-export-merge.test.ts`](file:///c:/Users/Mustafa/Desktop/mk%20whats/src/lib/contacts/contacts-export-merge.test.ts))**:
    - تغطية شاملة لـ 16 اختباراً مخصصاً شملت رفض الدمج الذاتي، عزل الحسابات، رفض المشاهدين، سياسة الدمج الحتمية، دمج المحادثات، حفظ الملاحظات والصفقات والطلبات والمواعيد، التراجع الكامل عند الخطأ، فحص باقة Excel، والتحقق من التوقيت المحلي وعلامة UTF-8 BOM.
    - اجتياز **686 اختباراً بنسبة 100% عبر 71 ملف اختبار** واجتياز الفحص المصدري الصارم لمترجم TypeScript (`tsc --noEmit`) بنجاح تام.
